    ...
    try {
        ...
        try {
            ...
            if (parsedResponse.needsInfoToAnswer) {
                ...
                // Simple exact match - just compare the strings
                const matchExactSlug = (searchSlug, targetSlug) => searchSlug === targetSlug;

                // Related match with normalization for fuzzy matching
                const matchRelatedSlug = (searchSlug, targetSlug) => {
                    ...
                    // Return true if more than 50% of parts match
                    ...
                    if (productSlug !== "None") {
                        ...
                        try {
                            ...
                            if (exactProducts.length > 0) {
                                ...
                                // Only if no exact matches found, try related matches
...
async function generateFullResponse(customerMessage, conversationHistory, productInfo, customerName) {
    try {
        ...
        const modelConfig = {
            ...
            max_tokens: 1000,
            temperature: storage.temperature !== undefined ? parseFloat(storage.temperature) : 0.7
        };
...
productInfo format: 
"product_slag"
["Variation Name", "price", "availability in our STORES" [0 or 1]],
<product_info>
${productInfo}
</product_info>
...
Format your response in(ckb or Arabic or English) inside <answer> tags. Remember to maintain a friendly, helpful, and sales-oriented tone throughout the interaction. you MUST keep your response under 200 characters,( if really need to or for special cases  400 characters).
<answer>
[your answer]
</answer>

**YOU MUST NEVER EVER USE THE WORD OUR WAREHOUSE OR EVEN REFERENCING TO WAREHOUSE**
